...
public interface BookMapper {
    ...
    @Insert("INSERT INTO book(title, author, isbn, total, available, create_time,category,borrow_count) " +
            "VALUES(#{title}, #{author}, #{isbn}, #{total}, #{available}, #{create_time}, #{category},#{borrow_count}")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Book book);

    @Update("UPDATE book SET title=#{title}, author=#{author}, isbn=#{isbn}, total=#{total}, " +
            "available=#{available}, create_time=#{create_time}, category=#{category},borrow_count=#{borrow_count} WHERE id=#{id}")
