...
public interface BookMapper {
    ...
    @Select("SELECT id,title,author,isbn,total,available,create_time,rating,category,borrow_count,cover,publisher,publish_date AS publishDate,description FROM book WHERE id = #{id}")
    ...
    @Insert("INSERT INTO book(title, author, isbn, total, available, create_time,category,borrow_count,cover,publisher,publish_date,description) " +
            "VALUES(#{title}, #{author}, #{isbn}, #{total}, #{available}, #{createTime}, #{category},#{borrowCount},#{cover},#{publisher},#{publishDate},#{description})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(Book book);

    @Update("UPDATE book SET title=#{title}, author=#{author}, isbn=#{isbn}, total=#{total}, " +
            "available=#{available}, create_time=#{createTime}, category=#{category},borrow_count=#{borrowCount},cover=#{cover}," +
            "publisher=#{publisher}, publish_date=#{publishDate}, description=#{description} WHERE id=#{id}")
